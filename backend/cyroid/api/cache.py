...
# Promote cached Docker image to VM Library as a snapshot

class PromoteToLibraryRequest(BaseModel):
    """Request to promote a cached Docker image to the VM Library."""
    image_name: str  # Full Docker image name (e.g., "ubuntu:22.04", "cyroid/kali-attack:latest")
    name: str  # Display name in the VM Library
    description: Optional[str] = None
    os_type: str = "linux"  # linux, windows, custom, network
    vm_type: str = "container"  # container, vm, windows
    default_cpu: int = 2
...
class PromoteToLibraryResponse(BaseModel):
    """Response after promoting an image to the VM Library."""
    id: str
    name: str
...
@router.post("/promote-to-library", response_model=PromoteToLibraryResponse, status_code=status.HTTP_201_CREATED)
def promote_image_to_library(
    request: PromoteToLibraryRequest,
    db: DBSession,
    current_user: AdminUser,
):
    """
    ...
    This creates a database record that allows the Docker image to be used
    as a source for creating new VMs in the VM Library.
    ...
    try:
        image = docker.client.images.get(request.image_name)
        image_id = image.id
        image_tag = image.tags[0] if image.tags else request.image_name
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Docker image '{request.image_name}' not found. Make sure it is cached first."
        )

    # Check if a snapshot with this name already exists
    existing = db.query(Snapshot).filter(Snapshot.name == request.name).first()
    if existing:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail=f"A snapshot with name '{request.name}' already exists in the VM Library."
        )
    ...
    snapshot = Snapshot(
        ...
        default_ram_mb=request.default_ram_mb,
        default_disk_gb=request.default_disk_gb,
        is_global=True,  # Make it visible to all users in the VM Library
        tags=request.tags,
        # vm_id is None since this is promoted from cache, not from a running VM
    )
    ...
    logger.info(f"Promoted Docker image '{request.image_name}' to VM Library as '{request.name}'")
    ...
    return PromoteToLibraryResponse(
        ...
        default_cpu=snapshot.default_cpu,
        default_ram_mb=snapshot.default_ram_mb,
        default_disk_gb=snapshot.default_disk_gb,
