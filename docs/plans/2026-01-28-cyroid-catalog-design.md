...
│   └── ...
├── templates/                    # VM template definitions (metadata only)
│   └── manifest.yaml
...
- **Hybrid layout**: Shared resources (`images/`, `templates/`) at the top level. Blueprint packages reference shared images by name rather than embedding Dockerfiles.
...
Item types: `blueprint`, `scenario`, `image`, `template`, `content`.
